...
object Main {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val appProperties:Properties = new Properties()

    val confSource = Source.fromFile("sparkConf.properties")
    confProperties.load(confSource.bufferedReader())

    val appSource = Source.fromFile("application.properties")
    appProperties.load(appSource.bufferedReader())


    val conf = new SparkConf()
      .setMaster(confProperties.getProperty("spark.master")).setAppName(confProperties.getProperty("spark.app.name"))
    val sc = new SparkContext(conf)
    //parsing JSON
    val sqlContext = new HiveContext(sc)

    Utility.decompressGzipFile("file.json", Utility.fileDownloader(appProperties.getProperty("downloadUrl")))
    ...
    if(!new File(appProperties.getProperty("output")).exists())
      new File(appProperties.getProperty("output")).mkdir()
    ...
    deleteFile(Utility.getFileNameFromURL(appProperties.getProperty("downloadUrl")))
